    ...
    client.onAddedToGroup(({ groupMetadata: { id }, contact: { name } }) =>
        client.getGroupMembersId(id)
            .then((ids) => {
                ...
                if (ids.length <= 10) {
                    client.sendText(id, 'Maaf Maksimum member grup hanya 10.').then(() => client.leaveGroup(id))
                } else {
                    client.sendText(id, `Halo group member *${name}*, terimakasih sudah menginvite bot ini, untuk melihat menu bot kirim: *#menu*`)
                }
            }))
